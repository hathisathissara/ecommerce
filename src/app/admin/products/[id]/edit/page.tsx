  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (productRes.ok) {
        ...
        setError(prodData.error || "Failed to update product");
      }
    } catch (err: unknown) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 min-h-screen">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-gray-900 tracking-tight">✏️ Edit Product</h1>
          <p className="text-xs text-gray-400 mt-0.5">Update product details, pricing, variants, and images.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-end pb-8">
          ...
          <button
            type="submit"
            disabled={loading}
            className="bg-black text-white px-10 py-3.5 rounded-xl font-bold text-xs uppercase tracking-wider hover:bg-gray-800 transition disabled:opacity-50"
          >
